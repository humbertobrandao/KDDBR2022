...
# This script summarize statistically data, which we create using Main_04 in 
...
auxTrain <- fread("../aux_04_train.csv");
auxTest <- fread("../aux_04_test.csv");

aux <- rbind(auxTrain, auxTest);

names(aux)[1] <- names(train)[1];
